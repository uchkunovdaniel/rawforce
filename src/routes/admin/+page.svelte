...
<main class="flex flex-col pt-10 items-center justify-start bg-(--pink) h-screen w-screen">
	...
	<section class="flex items-start bg-(--pink) justify-center {data.allOrders ? 'max-md:mt-16 md:mt-64 lg:mt-32' : ''}">
		{#if page === "orders"}
				...
				<main class="w-screen h-max flex justify-center p-10 items-baseline pt-32">
					<section class="grid lg:grid-cols-2 max-md:grid-cols-1 md:grid-cols-2 content-center gap-x-6 gap-y-16">
						{#each data.products as product (product.id)}
							...
							{/if}
						{/each}
						<article class="bg-pink-100 min-lg:col-span-3 h-64 md:col-span-2 rounded-xl flex max-md:flex-col max-md:max-w-64 items-center justify-around p-10 max-md:h-80">
							<img src={data.box.image} alt="cookies" class="w-70 h-60 max-md:w-30 max-md:h-40"/>
							<section class="flex flex-col items-center justify-center gap-4 z-50">
								<h3 class="text-oswald paragraph" style="color: var(--black)">{data.box.name}</h3>
								<BuyButton onclick={() => confirmDelete = true}>Премахни</BuyButton>
							</section>
							{#if confirmDelete}
